...
#[tauri::command]
fn get_current_display_baseline(
  app_handle: tauri::AppHandle,
) -> Option<crate::wayfern_manager::WayfernDisplayBaseline> {
  crate::wayfern_manager::WayfernManager::current_display_baseline(&app_handle)
}
...
pub fn run() {
  ...
  tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      legacy_cleanup::remove_legacy_cloud_state();
      ...
      if let Some(end) = content[start..].find("])") {
        ...
        for line in handler_content.lines() {
          ...
          if !line.is_empty() && !line.starts_with("//") {
            ...
            if !command.is_empty() {
              // Strip a module prefix from registered command paths.
